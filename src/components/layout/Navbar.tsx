          ...
          {mounted ? (
            <>
              <Link 
                ...
                className="hidden lg:flex items-center justify-center w-10 h-10 rounded-full border border-gray-200 bg-gray-50 hover:border-[#0c6a22] transition-all duration-300"
              >
                <svg 
                  className="w-5 h-5 text-gray-800 hover:text-[#0c6a22] transition-colors" 
                  fill="none" 
                  stroke="currentColor" 
                  viewBox="0 0 24 24"
                >
              ...
              <button onClick={() => setMenuOpen(!menuOpen)} className="lg:hidden text-black text-2xl z-50">
                {menuOpen ? "✕" : "☰"}
              </button>
            </>
          ) : (
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div 
            ...
            exit={{ opacity: 0, y: -20 }}
            className="lg:hidden absolute top-0 left-0 w-full h-screen bg-white flex flex-col items-center justify-center gap-8"
          >
            {links.map((link) => (
              <Link key={link.href} href={link.href} onClick={() => setMenuOpen(false)} className="text-3xl font-black text-black hover:text-[#0c6a22] transition-colors">
                {link.name}
              </Link>
            ))}
            <Link href="/login" onClick={() => setMenuOpen(false)} className="text-3xl font-black text-black hover:text-[#0c6a22]">Login</Link>
          </motion.div>
        )}
      </AnimatePresence>
    </motion.header>
